...
#
#       python ./download-all-2018-12-04_15-24-39.py
#
#    Download all files in a Metalink/CSV: (downloaded from ASF Vertex)
#
#       python ./download-all-2018-12-04_15-24-39.py /path/to/downloads.metalink localmetalink.metalink localcsv.csv
#
...
def cmdLineParse():
    ...
    Command Line Parser.
    '''
    parser = argparse.ArgumentParser(description='Downloading SAR data from ASF website')
    parser.add_argument('-i','--input', type=str, required=True, help='ASF format input file', dest='input')
    parser.add_argument('-d','--dir', type=str, required=True, help='Data directory', dest='dir')
    parser.add_argument('-a','--aux', type=str, required=False, help='AUX directory for SEN1A', dest='aux')
    parser.add_argument('-l','--link', type=str, required=False, help='AUX link download for SEN1A', dest='link')
    parser.add_argument('-p','--poeorb', type=str, required=False, help='POEORB directory for SEN1A', dest='poeorb')
...
if __name__ == "__main__":
   if len(sys.argv) < 3:
      print('Usage: download_ASFdata.py -i input -s SLC -a AUX -p POE')
      sys.exit()
   else:
      args = cmdLineParse()
      inp = args.input
   ...
   url_scene = data['URL'].tolist()
   date_scene = pd.to_datetime(data['Processing Date'])
   dtdelta = datetime.timedelta(days=20)
   date_process = date_scene + dtdelta
   yr = pd.DatetimeIndex(date_process).year
   mo = pd.DatetimeIndex(date_process).month
   ...
   if poeorb is not None:
      ...
      for i in range (0,len(url_scene)): 
         ...
         url = 'ftp://aux.sentinel1.eo.esa.int/POEORB/%s/%s/%s/S1A_\*.EOF' % (str(yr[i]),mo_str,dd_str)

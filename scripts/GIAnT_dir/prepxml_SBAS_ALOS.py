...
g = ts.TSXML('data')
g.prepare_data_xml('example.rsc', xlim=[0,int(width)], ylim=[0,int(height)], rxlim=None, rylim=None,latfile='lat.flt', lonfile='lon.flt', hgtfile='demfloat32.crop', inc = 21., cohth=0.2, chgendian='False', masktype='f4',unwfmt='RMG', demfmt='FLT', corfmt='RMG', endianlist=['UNW','COR','HGT'])
g.writexml('data.xml')

g = ts.TSXML('params')
g.prepare_sbas_xml(nvalid=int(j*0.8),netramp=True,atmos='',demerr=False,uwcheck=False,regu=True,masterdate='',filt=None)
g.writexml('sbas.xml')

...
def cmdLineParse():
    ...
    Command Line Parser.
    '''
    parser = argparse.ArgumentParser(description='Create ALOS input xmlfile for isceApp.py')
    ...
    parser.add_argument('-b','--bbox',type=str, default='./', help='Bounding box for DEM and SWBD', dest='bbox')
...
def isceApp_ALOS_xml_generator(data_root, date_list, datepairs, outdir):
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        print('Create_isce_ALOS_xml.py -r rawdir -c dateIdFile -o outdir')
        sys.exit()
    elif len(sys.argv) > 1:
        inputs = cmdLineParse()
        raw_root = inputs.raw
    ...
    isceApp_ALOS_xml_generator(raw_root, date_list, datepair_list, isceApp_xml_dir)
    swbd_xml_generator(bbox,isceApp_xml_dir)
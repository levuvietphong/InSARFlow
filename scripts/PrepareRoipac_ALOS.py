...
def extract_proc(mode,filexml):
    ...
    if mode=='insar':
        ...
        hdg = np.float(inp.runTopo.inputs.PEG_HEADING) * 180.0/np.pi
        wvl = np.float(inp.runTopo.inputs.RADAR_WAVELENGTH)
...
def extract_geoxml(mode,filexml):
...
if __name__ == '__main__':
    ...
    for line in file:
        ...
        if pmode=='insar':
            proc_list = iscedir+'/'+scene.split()[0]+'/insarProc.xml'
            geo_list = iscedir+'/'+scene.split()[0]+'/filt_topophase.unw.geo.xml'
        elif pmode=='stripmap':
            proc_list = iscedir+'/'+scene.split()[0]+'/stripmapProc.xml'
            geo_list = iscedir+'/'+scene.split()[0]+'/interferogram/filt_topophase.unw.geo.xml'
            fix_xml_stripmap(proc_list)     

        scene_full.append(scene)
        hdg, wvl, utc = extract_proc(pmode, proc_list)
        width, length, lon_delta, lon_start, lon_end, lat_delta, lat_start, lat_end = extract_geoxml(pmode,geo_list) 
        print(width,length,hdg,wvl,utc,scene.split()[0])
        
        # Fix stripmap filt_topophase xml error on LON_END value
        if pmode=='stripmap':
            lon_end = lon_start + lon_end * lon_delta
    ...
    if pmode=='stripmap':
        userfn = 'userfn_ALOS_stripmap.py'
    elif pmode=='insar':
        userfn = 'userfn_ALOS_insar.py'
    
    shutil.copy2(dir_path+'/scripts/GIAnT_dir/'+userfn,giant_dir+'/userfn.py')
    shutil.copy2(dir_path+'/scripts/GIAnT_dir/prepxml_SBAS_ALOS.py',giant_dir)